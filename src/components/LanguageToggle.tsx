...
export default function LanguageToggle() {
  ...
  return (
    <div className="flex items-center bg-white/5 border border-white/10 rounded-full p-0.5">
      <button
        onClick={() => setLang("es")}
        className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium transition-all min-h-[32px] ${
          lang === "es" ? "bg-white/10 text-white" : "text-gray-500 hover:text-gray-300"
        }`}
        aria-label="Español"
      >
        <svg className="w-5 h-3.5 rounded-sm overflow-hidden flex-shrink-0" viewBox="0 0 640 480">
